...
\begin{term}[Punti e link] \foreignlanguage{english}{Inner names} e porte vengono chiamati \emph{punti}; \foreignlanguage{english}{outer names} e archi vengono chiamati \emph{link}. Gli archi sono detti \emph{link aperti}, mentre gli outer names sono detti \emph{link chiusi}.
\end{term}
...
% aggiungere img placing elementari

Tutte le permutazioni possono essere costruite dalla simmetria elementare $\gamma_{1,1}$; tutti i placings possono essere costruiti a partire da $\gamma_{1,1}$, $1$ e $join$.

\begin{defi}[Linking, sostituzione, chiusura]
Un \emph{linking} ($\lambda$) è un bigrafo senza posti. I linkings sono generati per composizione, prodotto e identità a partire da \emph{sostituzioni} elementari $y/X$ e \emph{chiusure} elementari $/x:x\to \epsilon$.

% inserire img linkings elementari

Una \emph{sostituzione} ($\sigma$) è il prodotto di sostituzioni elementari; una \emph{chiusura} è il prodotto di chiusure elementari. La sostituzione vuota è $x:\epsilon \to x$. \qed
\end{defi}
...
C'è solo un tipo di bigrafo elementare che introduce nodi:
\begin{defi}[Ione]
...
\begin{defi}[(Bigrafo) Primo, discreto]
...
della teoria algebrica dei bigrafi. %bla bla bla + cite milner
...
\section{Evoluzione dei bigrafi}
...
Una segnatura è detta \emph{dinamica} se associa ad ogni controllo $K$ uno \emph{stato} nell'insieme $\{atomico,attivo,passivo\}$.
...
\begin{defi}[Regola di riscrittura]
...
\begin{oss}
Il fatto che $R$ e $R^\prime$ debbano avere la stessa interfaccia è un vincolo molto forte:
infatti non consente di fare, per esempio, la copia del contenuto di un nodo in un altro, o lo scambio
dei contenuti di due nodi in regioni differenti (mantenendo inalterata la loro posizione).
\end{oss}
Per rilassare questo vincolo, si va a definire una mappa tra le interfacce di redex e reactum.
...
\begin{term} Con la locuzione \emph{regola di riscrittura} si indicheranno le regole di riscrittura \emph{parametriche}.
\end{term}

\subsection{BRS - Sistema Reattivo Bigrafico}

\begin{defi}[Sistema Reattivo Bigrafico]
Un \emph{Sistema Reattivo Bigrafico} (BRS) è definito attraverso una coppia $(K , \mathcal{R})$, dove $K$  è una segnatura e $\mathcal{R}$ è un insieme di regole
di riscrittura; pertanto viene indicato con $BG(K,\mathcal{R})$.
 L'insieme $\mathcal{R}$ è chiuso rispetto all'\emph{equivalenza sul supporto}; ovvero,
se $R \bumpeq S$ e $R^\prime \bumpeq S^\prime$ e $(R,R^\prime,\eta) \in \mathcal{R}$ per un certo $\eta$, allora $(S,S^\prime,\eta) \in \mathcal{R}$.
\end{defi}
...
Nei capitoli a seguire, sarà illustrato un modo di implementare e simulare il BRS, nonché di ottenere tutti gli stati possibili da esso.
