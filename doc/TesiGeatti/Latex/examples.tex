...
Si progetti un algoritmo che eviti il \emph{deadlock} o la \emph{starvation}.
\end{prop}

Il problema chiede di progettare un algoritmo che eviti queste due situazioni:
\begin{itemize}
	\item
	ogni filosofo ha una forchetta e aspetta l'altra dal suo vicino: la situazioni si trova in uno stato di stallo (\emph{deadlock})
	\item
	una parte di filosofi riesce a mangiare e pensare ripetute volte, a discapito di un'altra parte che non riesce mai a mangiare perchè non ha mai due forchette, che muore di inedia (\emph{starvation}).
\end{itemize}
...
\caption{Regole per lasciare la forchetta \label{fig:dropFork}}
\end{figure}
...
\end{itemize}
per un totale di 6 stati possibili. Si vede bene come l'\textbf{esplosione combinatoria} aumenti in modo esponenziale in numero di nodi del grafo degli stati.
...
Number of nodes of the Model Checker:	50
\end{lstlisting}

Quello mostrato qui sopra è il risultato dell'esecuzione del problema: si è costruito un modulo che costruisse il bigrafo con un numero di filosofi scelto dall'utente. In questo caso si è scelto di avere cinque filosofi. In seguito, il sistema informa quale strategia si sta adottando. Nella terza riga ritorna il risultato, e nel caso ci sia un deadlock anche il numero di nodi del grafo degli stati.
