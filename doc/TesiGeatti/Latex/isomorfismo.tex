...
Si ï¿½ visto nel precedente capitolo come un bigrafo sia capace di evolversi all'interno di un BRS. In alcune situazioni, si vuole evitare evoluzioni infinite di un bigrafo, perchï¿½
per esempio porterebbero sempre a stati uguali fra di loro.

Per tenere traccia all'istante $t_k$ di tutti gli stati precedentemente assunti da un BRS ($t_0 \dots t_{k-1}$), si ï¿½ costruita una struttura dati a grafo, dove ogni nodo
ï¿½ uno stato, cioï¿½ un bigrafo: si chiamerï¿½ questo grafo  ``\emph{grafo degli stati}". Se un BRS parte dallo stato $S_0$, ï¿½ possibile che dopo $K$ regole di reazione lo stato $S_k$ sia uguale allo stato iniziale $S_0$. Questo significa che $S_0$ e $S_k$ hanno la stessa \emph{semantica}, cioï¿½ rappresentano lo stesso stato del sistema. Per cui, nel grafo degli stati, essi dovranno essere lo stesso medesimo nodo.

Questo ï¿½ equivalente a scrivere: ``se lo stato $S_{k}$ ï¿½ stato ottenuto da $S_{k-1}$ tramite la regola $R$ e $S_k$ e $S_0$ sono isomorfi, allora da $S_{k-1}$ si ottiene
$S_0$"
\begin{center}
se $S_{k-1}\stackrel{R}{\longrightarrow}S_{k}$ e $ S_k \bumpeq S_0 \qquad$ allora $S_{k-1}\stackrel{R}{\longrightarrow}S_{0}$
\end{center}
Capire quando due bigrafi sono isomorfi e quindi \emph{semanticamente equivalenti} ï¿½ di fondamentale importanza se si vogliono evitare i cosï¿½ detti \emph{loop} fra regole: per evitare che fra $S_{i}$ e $S_{i+1}$ si continuino ad applicare sempre le due stesse regole $R_1$ e $R_2$ all'infinito, devo capire che:
\begin{itemize} 
	\item
...
\end{itemize}
Questo concetto verrï¿½ chiarito con gli esempi sottostanti.
...
come mittente l'host $A$, esso arrivi al destinatario $B$. Ci saranno quindi delle regole di reazione per i router, che permetteranno di inoltrare i pacchetti verso le sue interfacce di uscita. Poichï¿½, utilizzando i soli bigrafi, servirebbero troppe regole di reazione per modellare il fatto che se il destinario ï¿½ $X$ allora l'interfaccia di uscita del pacchetto ï¿½ la numero $N$,  si puï¿½ pensare di inoltrare il pacchetto verso tutte le uscite in modo non deterministico. Cosï¿½ facendo, il pacchetto arriverï¿½ sicuramente al destinatario.
I problemi sono ora due:
\begin{itemize}
	\item
	il pacchetto arriverï¿½ a destinatari non corretti. Si puï¿½ introdurre una regola di reazione che elimini dall'host ogni pacchetto che non ha come destinatario l'host stesso.
	\item
	se prendiamo il $k$-esimo router $R_k$, allora il pacchetto ritornerï¿½ al $(k-1)$-esimo router $R_{k-1}$ , che a sua volta lo inoltrerï¿½ verso tutte le sue interfacce, e quindi 
	anche nuovamente verso $R_{k}$. Si ha cosï¿½ un ciclo infinito di pacchetti tra $R_k$ e $R_{k-1}$.
\end{itemize}

In questa sede si tratterï¿½ il secondo di questi problemi, che ha un'apparentemente semplice soluzione: capire quando due bigrafi sono uguali. In questo esempio, ï¿½ 
facile capire che per risolvere il problema basta verificare se gli stati $S_{k+i}$ e $S_{k-1}$ sono uguali: se $S_{k}$ ï¿½ il bigrafo in cui il pacchetto ï¿½ nel router $R_{k}$, allora
si applica la regola di inoltro e si generano tanti stati ($S_{k+1} \dots S_{k+j}$) quanti sono i router vicini a $R_k$; tra di questi ci sara anche $R_{k-1}$. Quindi, se supponiamo che nello stato $S_{k+i}$ il pacchetto torni indietro al router $R_{k-1}$, allora il problema ï¿½ capire che $S_{k+i}$ ï¿½ uguale $S_{k-1}$, cioï¿½ verificare che i due bigrafi siano isomorfi. In questo modo si sa che dallo stato $S_{k-1}$ non si dovrï¿½ piï¿½ applicare la regola di inoltro verso $S_{k}$, perchï¿½ questo causerebbe un ciclo infinito.

\section{Formulazione del problema}
Si ï¿½ visto che il concetto di isomorfismo ci aiuta a capire quando due bigrafi sono uguali. Riportiamo di nuovo la sua definizione formale:
\begin{define}[Isomorfismo]
Due bigrafi $F$ e $G$ si dicono isomorfi se e solo se esiste una traduzione di supporto tra $F$ e $G$, cioï¿½ se e solo se $F$ e $G$ sono \emph{support equivalent} ($F \bumpeq G$).
\end{define}

La definizione di isomorfismo puï¿½ far credere che esso sia un termine puramente sintattico, cioï¿½ che riguardi solamente la struttura dei due bigrafi. L'isomorfismo ï¿½ invece un'operazione che riguarda tanto la sintassi quanto la semantica: due bigrafi strutturalmente uguali sono due bigrafi semanticamente equivalenti (figura \ref{fig:bigMulIso}).

Il problema consiste quindi nel trovare un funzione biettiva che ha come dominio i nodi e gli archi del primo bigrafo, come codominio quelli del secondo bigrafo e che ne rispetti la struttura del primo. Come giï¿½ visto, tale funzione si chiama \emph{traduzione di supporto}.

Si noti come la traduzione di supporto consenta di ritenere isomorfi due bigrafi che sono uguali \emph{modulo permutazione}: tale funzione puï¿½ infatti operare una permutazione sui nomi dei nodi e degli archi, cosï¿½ come sugli inner names e outer names. In altre parole, non c'ï¿½ nessun vincolo sui nomi, ma solo sulla struttura del bigrafo. Si capirï¿½ meglio questo concetto negli esempi che seguiranno.


\subsection{Complessitï¿½}\label{sub:isoComplexity}
Stabilire la complessitï¿½ del problema dell'isomorfismo tra bigrafi ï¿½ molto importante, perchï¿½ relazionandolo con altri problemi ï¿½ possibile trovare un algoritmo efficiente per risolverlo. Facciamo un esempio: ipotizziamo che il problema dell'isomorfismo tra bigrafi (che chiameremo $P_{big}$) si trovi nella classe C, e che il problema $P_c$ sia \emph{completo} rispetto a questa classe. Per definizione di \emph{completezza}, ogni istanza di ogni problema appartenente alla classe C, quindi anche ogni istanza di $P_{big}$, puï¿½ essere tradotta (cioï¿½ \emph{ridotta}) con una certa complessitï¿½ al problema $P_c$.
...
Sia $P_{graph}$ il problema dell'isomorfismo tra grafi. Dato che $P_{graph}$ ï¿½ in NP ma non ï¿½ stato ancora dimostrato che sia completo, introduciamo la classe di complessitï¿½ GI, definita come l'insieme dei problemi che possono essere ridotti in tempo polinomiale al problema $P_{graph}$. Si dimostrerï¿½ il seguente risultato:

\begin{teor}[Complessitï¿½ di $P_{big}$]
Il problema dell'isomorfismo tra bigrafi ï¿½ GI-completo.
\end{teor}

\begin{dimos}
Per dimostrare l'equivalenza, procederemo in due direzioni: nella prima dimostreremo che esiste una riduzione in tempo polinomiale da $P_{graph}$ a $P_{big}$, cioï¿½ che ogni istanza del problema $P_{graph}$ puï¿½ venire tradotta in un'istanza del problema $P_{big}$. Invece, nella seconda direzione, faremo l'inverso. Si dimostreranno quindi che esistono le seguenti riduzioni:\\
...
\begin{itemize}
	\item
	$P_{graph} \le P_{big}$:\\
	Ogni grafo $G$ puï¿½ essere trasformato in un bigrafo $B$ tramite la seguente trasformazione. Innanzitutto, definiamo la segnatura di $B$: servono solamente due controlli, $Node$ con $0$ porte e $Port$ con una sola ($\{Node:0, Port:1\}$). Traduciamo ogni nodo $n_G$ di $G$ in un nodo $n_B$ di tipo $Node$ di $B$; in seguito, per ogni arco uscente o entrante di $n_G$ inseriamo in $n_B$ un nodo di controllo $Port$ e colleghiamo la sua porta all'altro nodo $n_B$ di $B$. Questa trasformazione consente quindi di trasformare ogni grafo in un bigrafo: dato che ï¿½ possibile implementarla come una visita sul grafo, questa riduzione ï¿½ in tempo polinomiale (\emph{Karp-riduzione} da  $P_{\pi}$ a $P_{big}$). Un esempio ï¿½ rappresentato in figura \ref{fig:firstRed}.
...
\end{figure}
	...
	%Dando una particolare segnatura al bigrafo, ogni termine del $\pi$-Calcolo puï¿½ essere tradotto in un bigrafo, quindi si ha che ogni istanza del problema $\pi$-SC ($\pi$  Structural Conguence), che controlla quando due termini sono equivalenti, puï¿½ essere tradotta in tempo polinomiale in un'istanza del problema $P_{big}$, che controlla quando due bigrafi sono isomorfi. Questa ï¿½ una \emph{Karp-riduzione} dal problema $P_{\pi}$ a $P_{big}$. In [...] si dimostra che il problema di congruenza tra termini del $\pi$-Calcolo, cioï¿½ $\pi$-SC, ï¿½ equivalente al problema dell'isomorfismo tra \emph{grafi}, $P_{graph}$. Quindi, abbiamo trovato una riduzione che traduce ogni istanza del problema di isomorfismo tra grafi in un'istanza del problema di isomorfismo tra bigrafi.
	
	\item
	$P_{big} \le P_{graph}$:\\
	Il primo approccio potrebbe essere quello di trattare i casi del place graph e del link graph separatamente, ma questa sarebbe una soluzione sbagliata: come si vedrï¿½ dagli esempi seguenti, due bigrafi che hanno sia place graph sia link graph isomorfi potrebbero non essere isomorfi. Per questo motivo, si deve trasformare ogni bigrafo in un grafo tramite questa trasformazione, di cui si ï¿½ riportato un esempio in figura \ref{fig:secondRed}. Ogni nodo, radice, sito, porta, innername, arco e outername viene trasformato in un nodo del grafo: in altre parole ogni entitï¿½ del bigrafo (compresi gli archi) viene trasformata in un nodo.
	
	Ora, l'idea ï¿½ quella di sfruttare le funzione $prnt$ e $link$ che compongono rispettivamente ogni place graph ed ogni link graph. Nel primo caso, se il nodo $n_1$ si trova all'interno di $n_2$ allora ci sarï¿½ un link tra questi due nodi. Nel secondo caso, se le porte $p1$ e $p_2$ sono collegate tramite l'arco $e_0$, allora nel grafo il nodo $e_0$ sarï¿½ collegato sia al nodo $p_1$ sia al nodo $p_2$. Come si vede bene dalla figura \ref{fig:secondRed}, si costruisce un grafo che tenga conto sia della struttura del place graph (archi rossi) sia di quella del link graph (archi verdi).
...
\end{figure}
	...
	%Procediamo ora con la seconda parte della dimostrazione: vogliamo dimostrare che ogni istanza del problema $P_{graph}$ puï¿½ venire tradotta in tempo polinomiale in un'istanza del problema $P_{big}$. Si ï¿½ visto nell'introduzione che ogni bigrafo ï¿½ decomponibile in due strutture tra loro ortogonali (place graph e link graph), e che quindi l'isomorfismo puï¿½ venire trattato separatamente per i due casi. Trattandosi di grafi, ogni istanza di $P_{big}$ puï¿½ venire ridotta a due istanza di $P_{graph}$. Abbiamo quindi trovato una riduzione da $P_{big}$ a $P_{graph}$.
\end{itemize}


Si ï¿½ quindi dimostrata l'equivalenza fra il problema dell'isomorfismo tra bigrafi e quella tra grafi, concludendo quindi che il problema $P_{big}$ ï¿½ equivalente al problema $P_{graph}$.
\end{dimos}

\begin{corol}
Il problema dell'isomorfismo tra bigrafi ï¿½ \textbf{equivalente} al problema dell'isomorfismo tra grafi.
\end{corol}


Quindi si ha che $P_{big} \in GI$. Come giï¿½ accennato, l'importanza di trovare la classe di complessitï¿½ a cui appartiene un dato problema ï¿½ enorme: se si trovasse un algoritmo che risolva in tempo polinomiale un problema NP-completo (cioï¿½ se la famosa domanda \emph{P is NP?} avesse risposta affermativa), allora tutti i problemi appartenenti alla classe NP, incluso $P_{big}$, potrebbero essere ridotti in tempo polinomiale al problema NP-completo di cui si ha l'algoritmo polinomiale. In questo modo, anche l'isomorfismo tra bigrafi sarebbe risolvibile in tempo polinomiale.


\section{Strategia di soluzione}
Dal punto di vista teorico, si ï¿½ trattato questo problema come una \emph{rete di flusso}: esse consentono di specificare in modo molto preciso delle condizioni sulla \emph{struttura} dei grafi. Qui l'obbiettivo ï¿½ trovare una traduzione di supporto, che conservi la struttura e sia biettiva: quest'ultima caratteristica per esempio puï¿½ essere espressa come il fatto che ogni nodo del primo bigrafo deve essere associato ad uno ed un solo nodo del secondo bigrafo. Tramite la rete di flusso, si puï¿½ specificare questa condizione dicendo che il flusso in uscita da ogni nodo deve essere esattamente 1. Questo ï¿½ un primo esempio del motivo per cui si ï¿½ scelta una rete di flusso per la modellizzazione.

Si ï¿½ scelto di trattare questo problema tramite la programmazione a vincoli. Questo paradigma permette di rappresentare al calcolatore un sistema di equazioni, che esso
risolverï¿½. Le equazioni saranno i vincoli che la rete di flusso dovrï¿½ rispettare. Nel nostro caso, ci basterï¿½ capire se esiste una soluzione al sistema, e non ci interessa sapere quali sono le sue soluzioni. Tutte le equazioni scritte sono lineari, con la conseguenza che si avrï¿½ un sistema lineare: questo permette di abbassare la complessitï¿½ dell'algoritmo.

Si ï¿½ scelta il paradigma del \emph{Constraint Programming} per i seguenti motivi:
\begin{itemize}
	\item
	...
	\item
	il motore che risolve il sistema di equazioni ï¿½ di solito soggetto a molti test, ed ï¿½ quindi affidabile. C'ï¿½ anche la possibilitï¿½ di imporre delle euristiche che migliorano notevolmente le prestazioni, rendendolo quindi anche efficiente.
	\item
	il motore puï¿½ venire considerato come una \emph{black box}, astraendosi quindi alla sua implementazioni interna, che puï¿½ per esempio venire cambiata o migliorata nel tempo, evitando di affliggere il software che lo usa.
\end{itemize}
E' possibile che esista un algoritmo che risolva in modo piï¿½ efficiente questo problema: se per esempio sarï¿½ inventato l'algoritmo che risolve in tempo polinomiale un isomorfismo tra grafi, allora per quanto dimostrato nella sottosezione \ref{sub:isoComplexity} anche questo problema potrï¿½ essere risolto in tempo polinomiale. Fino ad allora, la programmazione a vincoli, che puï¿½ non risultare il metodo migliore per risolvere questo problema, crea un ottimo compromesso tra complessitï¿½ dell'algoritmo e tempo di sviluppo.


Si ï¿½ precedentemente visto che ogni bigrafo ï¿½ formato da due strutture ortogonali e totalmente indipendenti, il place graph ed il link graph. Il problema dell'isomorfismo verrï¿½ quindi trattato
separatamente per le due strutture: ci saranno dei vincoli solamente per il place graph, ed altri solamente per il link graph. Infine, gli ultimi vincoli serviranno per conciliare le due soluzioni: si vedrï¿½ che senza di questi il calcolatore riconoscerï¿½ come uguali due bigrafi che hanno lo stesso link graph e lo stesso place graph modulo permutazione ma che non sono isomorfi (figura \ref{fig:falseIso}).


\subsection{Esempi}\label{sub:isoExamples}
Non ï¿½ sempre banale capire quando due bigrafi sono isomorfi. Si forniscono quindi alcuni esempi per acquisire familiaritï¿½ con questo concetto. 

Il primo riprende il bigrafo di figura \ref{fig:bigMul} per la moltiplicazione tra numeri naturali. Si vuole mostrare che, semanticamente, l'operazione $2*4$ ï¿½ equivalente
all'operazione $4*2$. In altre parole, non importa l'ordine dei nodi.

Incominciamo notando che i due link graph sono isomorfi, perchï¿½ non hanno archi e presentano lo stesso numero di nodi. Prendiamo ora in esame il place graph: se si ritorna alla sua definizione, si nota subito come i due place graph siano uguali perchï¿½ i nodi interni non sono ordinati.

Il fatto di dimostrare che i due bigrafi sono isomorfi e semanticamente equivalenti, dimostra che la proprietï¿½ commutativa della moltiplicazione vale anche nella sua versione bigrafica.\\
...
Presentiamo un altro esempio, questa volta su un caso negativo. Si ï¿½ specificato che per risolvere il problema si trattano separatamente i casi del place graph e del link graph: questo approccio perï¿½ necessita di avere dei vincoli di ``coerenza" che uniscano le due soluzioni. Senza questi vincoli, si puï¿½ incorrere nel seguente problema.
...
%%%%%%%%%			VINCOLI 				%%%%%%%%%%%
\section{Vincoli}
In questa sezione si presentano le equazioni necessarie per risolvere il problema dell'isomorfismo. Come giï¿½ anticipato, si lavorerï¿½ sulle \emph{reti di flusso}, che sono grafi orientati pesati. Distinguiamo le equazioni per il place graph, per il link graph e per la cosï¿½ detta coerenza.

\subsection{Vincoli per il place graph}
L'isomorfismo tra place graphs ï¿½ un isomorfismo tra foreste. Si vuole infatti vedere quando la prima foresta ï¿½ isomorfa alla seconda, a meno di permutazioni delle radici e dei siti. Una rete di flusso per questo problema ï¿½ quella in figura \ref{fig:placeFlow}.
...
Alle due foreste (cioï¿½ ai due place graph, costituiti da archi di colore verde), che chiameremo $P_F$ e $P_G$, si sono aggiunti altri archi (quelli rossi) \emph{solamente} tra nodi della stessa altezza, creando cosï¿½ un grafo. Questa ï¿½ la vera e propria rete di flusso per il problema dell'isomorfismo tra place graph. Ogni arco rosso ï¿½ \emph{orientato}, perchï¿½ va dai nodi della prima foresta ai nodi della seconda, e \emph{pesato}, perchï¿½ gli ï¿½ assegnato un numero naturale $p \in \{ 0, 1\}$.

Nell'implementazione, trattata tramite la programmazione a vincoli, ogni arco rosso ï¿½ una variabile. L'insieme delle variabili per il place graph sarï¿½ quindi:
\begin{center}
$M_{d,m,n} \in \{ 0, 1\} \qquad 0 \le d \le depth \qquad$\\ $\qquad \qquad m \in P_F^d$ \\ $\qquad \qquad n \in P_G^d$
\end{center}
dove \emph{depth} ï¿½ l'altezza massima della prima foresta, e $P_F^d$ e $P_G^d$ indicano l'insieme di nodi/radici/siti che si trovano all'altezza $d$ rispettivamente nel place graph $P_F$ e $P_G$.\\
In altre parole, creo una variabile che puï¿½ assumere valore 0 o 1 per ogni coppia di nodi $(a,b)$ che si trovano sulla stessa altezza, dove $a$ appartiene al primo place graph mentre $b$ al secondo.
...
La variabile $M_{d,m,n}$ assumerï¿½ il valore 1 se e solo se esiste una traduzione di supporto $\rho$ tale che $\rho(m)=n$.
\end{prop}
Tutte le altre variabili dovranno assumere il valore 0. Un altro modo di vedere la soluzione ï¿½ questa: la funzione di traduzione di supporto sarï¿½ definita da tutte e sole le variabili con valore 1. Infine: 
\begin{term}\label{term:matchVariabili}
Quando la variabile $M_{d,m,n}=1$ diremo che $m$ e $n$ costituiscono un match.
\end{term}

Nell'esempio di figura \ref{fig:placeFlow}, la soluzione esiste ed ï¿½ definita come in figura \ref{fig:placeFlowRis}, dove i numeri sopra le variabili (archi rossi) indicano i valori che esse hanno assunto dopo la risoluzione del sistema di equazioni.
...
Incominciamo con i \textbf{vincoli strutturali}: si ï¿½ adottata una versione ricorsiva per i vincoli. Distinguiamo quindi in caso base e passo ricorsivo.
\begin{itemize}
	\item
	...
	\end{center}
	Nella figura sottostante, si vede subito che la variabile $M_{0,r_0,r_0}=0$ perchï¿½ $prnt_F^{-1}(r_0)=1 \ne 2=prnt_G^{-1}(r_0)$.
	...
	\end{center}
	Esso costituisce il caso ricorsivo perchï¿½ vale per tutti i nodi tranne le radici, e le radici sono ricoperte dal caso base. Si veda la figura \ref{fig:secondConstraintExe}: la variabile 
	$M_{1,n_1,n_2}=0$ perchï¿½ dalla figura \ref{fig:firstConstraintExe} sappiamo che $M_{0,r_0,r_0}=0$ e perchï¿½ $prnt_F(n_1)=r_0$ e $prnt_G(n_2)=r_0$. Lo stesso ragionamento vale per la variabile $M_{1,n_1,n_3}$.
...
Questi vincoli non sono perï¿½ sufficienti per determinare un isomorfismo tra i due place graph $P_F$ e $P_G$. In particolare, la traduzione di supporto deve essere biiettiva, cioï¿½ associare un nodo/radice/sito di $P_F$ ad uno ed un solo nodo/radice/sito di $P_G$. Si sono quindi aggiunti i \textbf{vincoli di flusso}.
...
\begin{itemize}
	...
	\end{notaz}
	Esso si traduce nel fatto che la somma di tutte le variabili in uscita da ogni nodo/radice/sito di $P_F$ deve essere 1, ovvero: ogni nodo di F puï¿½ costituire un match solamente con uno ed un solo altro nodo di G. In formule:
	\begin{center}
	...
	\end{figure*}
	In figura \ref{fig:thirdConstraintExe} si puï¿½ vedere come la somma degli archi in uscita dal nodo $n_1$ sia 1, ovvero: 
	...
	\end{notaz}
	Esso ï¿½ equivalente a dire che la somma di tutte le variabili in entrata da ogni nodo/radice/sito di $P_G$ deve essere 1. In altre parole: ogni nodo di G puï¿½ costituire un match solamente con uno ed un solo altro nodo di F. In formule:
	\begin{center}
	...
	\end{figure*}
	In figura \ref{fig:fourthConstraintExe}, ï¿½ stato espresso il vincolo che il flusso in entrata verso il nodo $n_1$ deve essere 1, cioï¿½: 
...
L'isomorfismo tra link graph ï¿½ un isomorfismo tra ipergrafi. In questo problema, sfruttiamo soprattutto la definizione di link graph: la sua struttura ï¿½ definita dalla funzione \emph{link}, che collega Punti a Handles. I primi sono l'insieme delle porte e degli inner names, mentre i secondi sono l'insieme degli archi e degli outer names (sottosezione \ref{sub:linkGraph}). Si puï¿½ quindi vedere ogni link graph come una funzione che ha come dominio i Punti e come codominio gli Handles. 

Sulla base di queste osservazioni, possiamo costruire la \emph{rete di flusso} per questo problema nel seguente modo: innanzitutto chiamiamo i due link graph rispettivamente $L_F$ e $L_G$, e le loro funzioni come $link_F$ e $link_G$ (definite dagli archi verdi di figura \ref{fig:linkFlow}). Possiamo collegare tutti gli elementi del dominio di $link_F$ a tutti gli elementi del dominio di $link_G$, creando cosï¿½ archi \emph{orientati}, perchï¿½ vanno da punti di $L_F$ a punti di $L_G$, e \emph{pesati}, perchï¿½ possono assumere un valore $p \in \{0, 1\}$.
...
Si ï¿½ cosï¿½ creata la rete di flusso in figura \ref{fig:linkFlow}, dove $D_X$ (con $X \in \{ F, G\}$) indica il dominio di $link_X$ e $C_X$ il suo codominio. \\
...
I vincoli che si devono scegliere hanno il compito di ``costituire" la funzione di traduzione di supporto, e devono quindi assicurare che essa sia biiettiva e che mantenga la struttura del primo link graph. Perciï¿½, anche in questo caso, distinguiamo in \emph{vincoli di flusso}, per il primo problema, e in \emph{vincoli strutturali}, per il secondo.

Incominciamo con il definire i \textbf{vincoli strutturali}. Essi hanno il compito di definire una funzione che va dal primo link graph al secondo, che sia in grado di mantenere la struttura del primo. In altre parole, devono controllare che le due strutture siano \emph{compatibili}. I vincoli strutturali fanno riferimento al caso negativo, cioï¿½ descrivono nel sistema quando due punti o due handle \emph{non} possono costituire un match. Da qui i due vincoli:

\begin{itemize}
	\item
	\emph{Primo vincolo strutturale}: due handles (il primo di $L_F$ e il secondo di $L_G$) che hanno un numero diverso di pre-immagini \emph{non} possono costituire un match. In altre parole: se l'handle $h$ ï¿½ immagine di due punti ma l'handle $h'$ lo ï¿½ di uno solo, allora $h$ e $h'$ non possono essere associati. In formule:
	\begin{center}
	$x_{h,h'} = 0 \qquad \qquad \qquad |link^{-1}_F(h)| \ne |link^{-1}_G(h')| \qquad $ \\ 
	$\qquad \qquad h \in C_F = Y_F \uplus E_F$ \\ 
	$\qquad \qquad h' \in C_G = Y_G \uplus E_G$
	\end{center}
	In figura \ref{fig:firstLinkConstraint}, si puï¿½ vedere come la variabile $x_{h_3,h_1}$ sia vincolata ad assumere il valore 0, infatti: $|link_F^{-1}(h_3)| = 0$ ma 
	...
	\end{center}
	dove $p \in link^{-1}_F(h)$ e $p' \in link^{-1}_G(h')$. Esso puï¿½ essere tradotto tramite la seguente equazione.
	\begin{center}
	...
	In figura \ref{fig:secondLinkConstraint}, si puï¿½ vedere un esempio per questo vincolo. Dal primo vincolo sappiamo che $x_{h_2,h_1} = 0$ perchï¿½ 
...
Descriviamo adesso i \textbf{vincoli di flusso}, che consentono di avere una funzione biiettiva. Il caso ï¿½ analogo a quello per il place graph e la notazione rimane la stessa.
...
\begin{itemize}
	...
	\end{figure*}
			...
			\item
			\emph{$\delta^+(h)=1$}:
			E' l'analogo del caso precedente. Ogni handle di $C_F$ deve essere associato ad uno e un solo handle di $C_G$. La rispettiva equazione ï¿½:
				\begin{center}
	...
	\emph{Flusso in entrata}:
		\begin{itemize}
			\item
			\emph{$\delta^-(p')=1$}: ogni punto di $D_G$ puï¿½ essere associato ad uno e un solo punto di $D_F$. In formule:
				\begin{center}
				$\sum\limits_{p}y_{p,p'} = 1 \qquad \qquad \qquad p \in X_F \uplus P_F \qquad p' \in X_G \uplus P_G$
				\end{center}
			...
			\item
			\emph{$\delta^-(h')=1$}: ogni handle di $C_G$ puï¿½ essere associato ad uno e un solo handle di $C_F$. In formule:
				\begin{center}
...
Si noti che i quattro vincoli di flusso assicurano la iniettivitï¿½ e la suriettivitï¿½ della funzione di traduzione di supporto, rendendola quindi \emph{biiettiva}.


\subsection{Vincoli di coerenza}
Nella sottosezione \ref{sub:isoExamples}, dedicata ad alcuni esempi, la figura \ref{fig:falseIso} mostrava due bigrafi che, pur avendo i place graphs ed i link graphs isomorfi, \emph{non} erano tali. Questo perchï¿½ il sistema di equazioni non era completo: non bastano cioï¿½ i vincoli visti fino ad ora, e bisogna integrarli con dei \textbf{vincoli di coerenza} che sono dedicati all'unione delle due soluzioni.

Nell'esempio citato, si era giï¿½ notato informalmente che il quadrato collegato al cerchio nel primo link graph deve essere quello piï¿½ esterno nel primo place graph. Diamo ora le definizioni formali di questi vincoli:
...
\begin{itemize}
	...
	\emph{Primo vincolo di coerenza}: due porte costituiscono un match nei due link graphs \emph{se e solo se} i rispettivi nodi lo fanno nei due place graphs e le due porte hanno lo stesso indice. In formule:
		...
		$i \in \mathbb{N}$ 
		\end{center}
	In altre parole, se due nodi $m$ e $m'$ non costituiscono un match nei due place graphs, allora neanche le loro porte lo fanno. D'altra parte, se $m$ e $m'$ costituiscono un match, cioï¿½ $M_{d,m,m'}=1$, allora le porte con lo stesso indice devono costituire un match: $y_{p,p'} = 1$.
	...
	\item
	\emph{Terzo vincolo di coerenza}: se due punti sono di tipo diverso, cioï¿½ il primo ï¿½ una porta e il secondo un inner name, allora essi non possono costituire un match.
		\begin{center}
		$y_{p,p'} = 0 \qquad \qquad \qquad p \in P_F \qquad p' \in X_G \qquad $ \\ 
		\end{center}
		Banalmente, vale anche il caso speculare, dove il primo punto ï¿½ un inner name mentre il secondo ï¿½ una porta:
		\begin{center}
		$y_{p,p'} = 0 \qquad \qquad \qquad p \in X_F \qquad p' \in P_G \qquad $ \\ 
		\end{center}
		
	\item
	\emph{Quarto vincolo di coerenza}: se due handles sono di tipo diverso, cioï¿½ il primo ï¿½ un arco e il secondo un outername, allora essi non possono costituire un match.
		\begin{center}
		$y_{h,h'} = 0 \qquad \qquad \qquad h \in E_F \qquad h' \in Y_G \qquad $ \\ 
		\end{center}
		Il caso speculare, dove il primo handle ï¿½ un outer name name mentre il secondo ï¿½ un arco, ï¿½ dato dalla formula:
		\begin{center}
		$y_{h,h'} = 0 \qquad \qquad \qquad h \in Y_F \qquad h' \in E_G \qquad $ \\ 
		\end{center}
	
	\item
	\emph{Quinto vincolo di coerenza}: un nodo puï¿½ costituire un match solo con un altro nodo. Lo stesso vale per le radici e per i siti. Perciï¿½, ï¿½ impossibile per esempio che esista una variabile con valore 1 da una radice verso un nodo. I vincoli che coprono tutti i possibili casi sono:
		\begin{center}
		...
		\end{center}
	\item
	\emph{Sesto vincolo di coerenza}: due nodi con controlli diversi non possono costituire un match. Per esempio, in figura \ref{fig:falseIso} il quadrato piï¿½ grande del primo bigrafo non puï¿½ essere associato con il cerchio del secondo.
	\begin{center}
...
Tutti i vincoli sono stati espressi sul calcolatore tramite la libreria Java \emph{Choco v3.3.1}. Essa permette di rappresentare un sistema di equazioni che poi la stessa libreria risolverï¿½. Forniamo quindi una panoramica della libreria ed alcuni esempi di vincoli che sono stati espressi.
...
\end{lstlisting}
Si noti come ogni variabile venga associata ad un \textbf{solver}, che ï¿½ il motore interno che risolverï¿½ il sistema. Sia $x$ che $y$ sono associate allo stesso solver: questo equivale a dire che apparterranno allo stesso sistema di equazioni. La sintassi dei metodi ï¿½ chiara: come primo argomento c'ï¿½ il nome della variabile, come secondo ci puï¿½ essere un vettore (che enumera tutti gli elementi del dominio) oppure il primo estremo dell'intervallo del dominio; come terzo argomento abbiamo il limite superiore del dominio, ed infine c'ï¿½ il solver al quale le variabili dovranno fare riferimento.
...
\end{cases}$
\end{center}
si puï¿½ rappresentare con le seguenti istruzioni:\\
\begin{lstlisting}
IntVar[] sumV = {x,y};
...
Si noti come tutti i vincoli facciano riferimento a ``solver", e che quindi faranno parte dello stesso sistema. Inoltre, con il metodo $sum$ si ï¿½ dovuto creare una costante che rappresentasse il numero 7 (nel metodo $arithm$ questo non ï¿½ necessario). Vediamo brevemente come si usano questi metodi: il metodo $fixed$ serve per creare una costante; ï¿½ simile ad $enumerated$ e vuole come argomenti il nome della variabile, il valore della costante ed il solver al quale fare riferimento.
...
Il metodo $arithm$ consente di esprimere le piï¿½ comuni equazioni aritmetiche: il primo argomento ï¿½ una variabile, il secondo un operatore ($<,\le,=,\ge,>$) ed il terzo una variabile o un numero naturale.

Con il seguente comando: 
\begin{lstlisting}
solver.findSolution();
\end{lstlisting}
la libreria cerca di risolvere il sistema di equazioni. Se esiste una soluzione ritorna True, altrimenti False. Si noti come in questa sede non ci interessino le soluzioni del sistema, che perï¿½ potrebbero venire calcolate con il metodo:
\begin{lstlisting}
var.getValue() //dove var ï¿½ una IntVar
\end{lstlisting}
per ogni variabile $var$ che fa parte del sistema.

Da quanto appena visto, l'implementazioni dei vincoli per l'isomorfismo tra bigrafi ï¿½ una traduzione quasi immediata delle formule della sezione precedente. Forniamo quindi solo alcuni esempi:
\begin{itemize}
	\item
	Il vincolo sul flusso in uscita ($\delta^+(p)=1$) da ogni punto del primo link graph ï¿½ $\sum\limits_{p'}y_{p,p'} = 1$. La sua traduzione nella libreria \emph{Choco vs 3.3.1} usa il metodo \emph{sum} visto prima. Si guardi la figura \ref{fig:outFlowChoco}: se vogliamo esprimere questo vincolo su $p_3$ allora la corrispondente istruzione sarï¿½: 
	\begin{lstlisting}
	...
	Come si puï¿½ vedere, la traduzione ï¿½ quasi immediata: il metodo $arithm$ consente di scrivere direttamente l'equazione, risultando quindi molto comodo.
...
Nella sottosezione \ref{sub:isoComplexity} si ï¿½ vista la complessitï¿½ teorica del problema dell'isomorfismo tra bigrafi. Ora si vedranno dei test (benchmark) volti a misurare le prestazioni dell'algoritmo proposto. Tutti i test sono stati eseguiti su una macchina con queste caratteristiche:
\begin{itemize}
	\item
...
La preparazione dei dati ï¿½ stata condotta con attenzione: si ï¿½ implementato un metodo che potesse generare bigrafi casuali con il numero di nodi richiesto, per fare in modo che il tempo di risoluzione non dipendesse dalla struttura del bigrafo, che puï¿½ essere semplice o complessa. Questo metodo ï¿½ stato implementato in un modo molto semplice: \begin{itemize}
	\item
	per prima cosa si ï¿½ costruito il place graph, che ï¿½ equivalente a generare un albero casuale. All'inizio del metodo, si puï¿½ specificare il numero di porte che gni nodo dell'albero possiede.
	\item
	in seguito, ogni porta di ogni nodo del place graph ï¿½ stata collegata ad un outername scelto casualmente da un insieme precedentemente generato.
\end{itemize}

Si vedranno due grafici: 
\begin{itemize}
	\item
	il primo rappresenta il \emph{tempo di preparazione} (figura \ref{fig:loadingTime}): ï¿½ la misura di quanto l'algoritmo impiega per creare tutte le variabili ed i vincoli
	\item
	il secondo rappresenta il \emph{tempo d'esecuzione} (figura \ref{fig:workingTime}), cioï¿½ quanto tempo impiega la libreria \emph{Choco} per risolvere il sistema di equazioni
\end{itemize}
...
Ogni grafico ha sulle ascisse il numero di nodi del problema: in altre parole, il problema testato in $x=n$ verifica l'isomorfismo tra due bigrafi con $n/2$ nodi ciascuno. Sulle ascisse c'ï¿½ invece il tempo espresso in secondi. Il secondo grafico ï¿½ in scala logaritmica: l'aumento del numero di nodi causa un esplosione combinatoria tra i due bigrafi, che porta ad un aumento esponenziale del numero di vincoli e quindi anche del relativo tempo d'esecuzione.\\
