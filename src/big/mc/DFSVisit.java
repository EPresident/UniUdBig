...
/**
 * This is a VISIT, not a strategy. There are two ways (methods) to start a
 * visit: - startFromRoot(), it starts the visit from the root of the
 * BigStateGraph. It produces a tree. - start(BSGNode), it starts the visit from
 * the node specified. It produces a forest (generally).
 *
...
public class DFSVisit implements Visit {
     ...
     * @return true if at least one node satisfies the predicates, false
     * otherwise.
     */
    @Override
    ...
    /**
     * Checks if the graph and predicate are valid
     *
     * @author Elia
     */
    private boolean valid() {
        if (bsg == null) {
            System.err.println("Error in DFSVisit: null BSG.");
            return false;
        }
        if (predicate == null) {
            System.err.println("Error in DFSVisit: null predicate.");
            return false;
        }
        if (bsg.getGraphSize() <= 0) {
            System.err.println("Warning in DFSVisit: empty BSG.");
            return false;
        }
        return true;
    }
